...
import { AlertCircle, CheckCircle, Package, ArrowRight, Refrigerator } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ inventory, lang, onNavigate }) => {
  const now = Date.now();
  const threeDaysInMs = 3 * 24 * 60 * 60 * 1000;

  const totalItems = inventory.length;
  const expiredItems = inventory.filter(i => i.expiryDate < now).length;
  const expiringSoonItems = inventory.filter(i => i.expiryDate >= now && i.expiryDate <= now + threeDaysInMs).length;
  // Get actual expiring items for display
  const expiringList = inventory
    .filter(i => i.expiryDate >= now && i.expiryDate <= now + threeDaysInMs)
  ...
  return (
    <div className="space-y-8">
      ...
      <header className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-black text-gray-800 tracking-tight">{t.greeting}</h1>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white p-5 rounded-3xl shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-gray-50 h-32 flex flex-col justify-between">
          ...
          <div>
            ...
            <span className="text-xs text-gray-400 font-medium">{t.expiring}</span>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        {expiringSoonItems === 0 && expiredItems === 0 ? (
          ...
          <div className="bg-white rounded-3xl p-8 border border-dashed border-gray-200 flex flex-col items-center justify-center text-center shadow-sm">
            ...
            <p className="text-gray-400 font-medium text-lg">{t.goodState}</p>
          </div>
        ) : (
          ...
          <div className="space-y-3">
             {expiredItems > 0 && (
                <div className="bg-red-50 border border-red-100 p-4 rounded-2xl flex items-center justify-between">
                  <span className="text-red-600 font-bold flex items-center gap-2">
                    <AlertCircle size={18} /> {expiredItems} {t.expired}
                  </span>
                  <button onClick={() => onNavigate('inventory')} className="text-xs bg-white text-red-600 px-3 py-1.5 rounded-full font-bold shadow-sm">
                    {t.check}
                  </button>
                </div>
             )}
             ...
             {expiringSoonItems > 3 && (
               <button onClick={() => onNavigate('inventory')} className="w-full text-center text-sm text-gray-400 py-2 hover:text-emerald-500">
