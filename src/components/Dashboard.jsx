...
export default function Dashboard({ items }) {
    ...
    return (
        <div className="p-6 space-y-6">
            <header>
                <h1 className="text-2xl font-bold text-gray-800">Good Day! ☀️</h1>
                <p className="text-gray-500">Here's what's happening in your fridge.</p>
            </header>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className={`p-4 rounded-2xl border ${expiringSoonCount > 0 ? 'bg-orange-50 border-orange-100' : 'bg-green-50 border-green-100'}`}>
                    <p className={`${expiringSoonCount > 0 ? 'text-orange-600' : 'text-green-600'} font-medium text-sm`}>
                        {expiringSoonCount > 0 ? 'Expiring Soon' : 'All Good'}
                    </p>
                ...
                <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100">
                    <p className="text-blue-600 font-medium text-sm">Total Items</p>
                    <p className="text-3xl font-bold text-blue-800 mt-1">{items.length}</p>
                    <p className="text-xs text-blue-400 mt-1">In your inventory</p>
                </div>
            </div>
            ...
            {expiredCount > 0 && (
                <div className="bg-red-50 p-4 rounded-2xl border border-red-100 flex items-center justify-between">
                    <div>
                        <p className="text-red-800 font-bold">⚠️ {expiredCount} Expired Items</p>
                        <p className="text-red-500 text-xs">Please check and remove them.</p>
                    </div>
                </div>
            )}
            ...
            <section>
                ...
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm text-center">
                    <p className="text-gray-500 text-sm mb-4">Planning a meal?</p>
